...
import { memoSearchNotesParametersSchema } from './memo-search-params.js';
...
server.addTool({
  description:
    'Search notes in reminds by semantic similarity. Returns a list of note gids. ' +
    'Optional filters narrow results by time range, tags, pinned state, attachments, ' +
    'recycle state, or review counts.',
  execute: async (params) => {
    const mode = params.mode ?? 'retrieval';
    const gids = await searchNotes(
      params.query,
      mode,
      params.limit,
      params.filters,
  ...
  name: 'search_notes',
  parameters: memoSearchNotesParametersSchema,
});
